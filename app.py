...
def get_speech_for_grade(grade, speech_type, extracted_data):
    ...
    templates = {
        ...
        "6-7年级": {
            "完整优秀": f"我认为这件事的核心不是简单的赔偿问题，而是关乎责任认知与人际修复的深层议题。首先，{name}需要完成一次真诚的自我反思——不是机械地说'对不起'，而是真正理解自己的行为对他人造成了什么样的影响。其次，双方需要开放地沟通各自的需求与期待。最后，双方共同制定一个具体的、可执行的修复方案。在这个过程中，{name}展现的不只是认错的态度，更是一种成熟的责任担当。",
            ...
            "单维度深挖": f"我认为这个案例最值得深思的是'同理心'在冲突解决中的核心作用。{name}需要的不是一套应付式的道歉模板，而是真正进入对方的精神世界——去感受对方投入的时间、精力和情感期待。当一个人能够站在对方的立场，体验到'失去'对他人意味着什么，他的道歉才能真正触及问题的本质，解决方案也才能真正被双方共同接纳。"
        }
    }
    ...
    if st.button("🔬 开始诊断并自动优化", type="primary", use_container_width=True):
        ...
        for _ in range(samples_per_type):
            for speech_type in test_types:
                ...
                speech_desc = next(t["desc"] for t in SPEECH_TYPES_CONFIG if t["id"] == speech_type
